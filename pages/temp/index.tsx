...
import { BadgeVcCard } from "../../components/temp/ui/BadgeVcCard";
import { useForm } from "react-hook-form";
import { SearchForm } from "../../components/temp/ui/searchForm";
import { SearchFormItem } from "../../types/temp";
import { ArrowLeftIcon, ArrowRightIcon, ChevronLeftIcon, ChevronRightIcon } from "@chakra-ui/icons";
import { Pagination } from "../../components/temp/ui/Pagination";
...
const Home: NextPage<Props> = (props) => {
  ...
  const handleClickPrev = async () => {
    console.log("previod");
    const res = await fetch("http://localhost:3003/api/temp/badgeVcList");
  ...
  const handleClickNext = async () => {
    console.log("next");
    const res = await fetch(`http://localhost:3003/api/temp/badgeVcList?page=${totalPages}`);
    const data = await res.json();
    setPageState(data);
  };
  ...
  const handleClickMove = async (page: number) => {
    ...
    const res = await fetch(`http://localhost:3003/api/temp/badgeVcList?page=${page}`);
    const data = await res.json();
    setPageState(data);
  };
...
export const getServerSideProps: GetServerSideProps = async () => {
  const data = await fetch("http://localhost:3003/api/temp/badgeVcList");
